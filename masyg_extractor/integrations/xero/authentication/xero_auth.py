...
async def callback(request: Request):
    ...
    print("xero session auth", access_token, refresh_token)

    id_token = response_json.get("id_token")  # Optional: contains user identity information
    print("xero session auth", access_token, refresh_token, id_token)
    ...
    request.session["xero_access_token"] = access_token
    request.session["xero_refresh_token_encrypted"] = encrypt_token(refresh_token)
    request.session["xero_id_token"] = id_token  # Optional: store the id_token if needed
...
async def connections(request: Request):
    ...
    """
    if "xero_access_token" not in request.session:
        return RedirectResponse("/integrations/xero/auth/login")

    headers = {
        "Authorization": f"Bearer {request.session['xero_access_token']}",
        "Content-Type": "application/json"
    }
    connections_url = "https://api.xero.com/connections"
...
async def refresh_xero_token(request: Request):
    refresh_token_encrypted = request.session.get("xero_refresh_token_encrypted")
    ...
    if "access_token" in response_data:
        ...
        request.session["xero_access_token"] = access_token
        request.session["xero_refresh_token_encrypted"] = encrypt_token(new_refresh_token)
