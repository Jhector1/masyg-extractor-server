...
async def callback(request: Request):
    ...
    if not access_token or not refresh_token:
        return JSONResponse({"error": "Failed to obtain tokens."}, status_code=status.HTTP_400_BAD_REQUEST)

    request.session["access_token"] = access_token
    request.session["refresh_token_encrypted"] = encrypt_token(refresh_token)
...
async def refresh_quickbooks_token(request: Request):
    refresh_token_encrypted = request.session.get("refresh_token_encrypted")
    ...
    if "access_token" in response_data:
        ...
        request.session["access_token"] = access_token
        request.session["refresh_token_encrypted"] = encrypt_token(new_refresh_token)
        return nocache_response(
            JSONResponse({"message": "Token refreshed successfully.", "access_token": access_token}))
    else:
        error = response_data.get("error")
        if error == "invalid_grant":
            return nocache_response(JSONResponse({"error": "Session expired. Please reauthenticate."}, status_code=401))
        return nocache_response(
            JSONResponse({"error": "Token refresh failed.", "details": response_data}, status_code=400))

# Now, instead of creating a new FastAPI instance, include this router in your existing one.
