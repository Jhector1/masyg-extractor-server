...
from masyg_extractor.services.progress_log import get_file_progress_dict, calculate_overall_progress, \
    safe_emit_progress, TEXT_EXTRACTION_WEIGHT, GPT_PROCESSING_WEIGHT, FILE_READ_WEIGHT, COMPRESSION_WEIGHT, \
    FIRESTORE_UPDATE_WEIGHT
...
async def extract_text(
    ...
    for step in range(extraction_steps):
        ...
        progress["text_extraction"] = ((step + 1) / extraction_steps) * TEXT_EXTRACTION_WEIGHT
        await safe_emit_progress(client_id, calculate_overall_progress(progress))
    ...
    while extractor_index < len(extractors):
        ...
        if extracted_text and extracted_text.strip():
            ...
            break
        extractor_index += 1
    progress["text_extraction"] = TEXT_EXTRACTION_WEIGHT
    await safe_emit_progress(client_id, calculate_overall_progress(progress))
...
async def process_text_and_parse(
    ...
    progress["gpt_processing"] = GPT_PROCESSING_WEIGHT
    await safe_emit_progress(client_id, calculate_overall_progress(progress))
...
async def process_file_async(
    ...
    try:
        ...
        for step in range(read_steps):
            ...
            progress["file_read"] = ((step + 1) / read_steps) * FILE_READ_WEIGHT
            await safe_emit_progress(client_id, calculate_overall_progress(progress))
        ...
        for step in range(comp_steps):
            await asyncio.sleep(0.2)
            progress["compression"] = ((step + 1) / comp_steps) * COMPRESSION_WEIGHT
            await safe_emit_progress(client_id, calculate_overall_progress(progress))
        ...
        for step in range(fs_steps):
            await asyncio.sleep(0.2)
            progress["firestore_update"] = ((step + 1) / fs_steps) * FIRESTORE_UPDATE_WEIGHT
            await safe_emit_progress(client_id, calculate_overall_progress(progress))
...
async def process_file_wrapper(
    idx: int, file, user_id: str, group_id: str, client_id: str, global_progress: Dict[str, float], files_count: int
) -> Tuple[int, str, Any]:
    parsed_content, sanitized_filename = await process_file_async(file, user_id, group_id, client_id, global_progress, files_count)
    return idx, sanitized_filename, parsed_content

async def process_files_in_parallel(
    files: List, user_id: str, group_id: str, client_id: str, global_progress: Dict[str, float]
) -> Dict[int, Any]:
    total_files = len(files)
    file_progress_trackers = {idx: get_file_progress_dict() for idx in range(total_files)}

    tasks = [
        process_file_wrapper(idx, f, user_id, group_id, client_id, global_progress, total_files)
