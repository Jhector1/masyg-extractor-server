...
async def process_chunk(chunk_text: str, progress_logger: ExtractorProgressLog, file_progress_share: float):
    ...
    SYSTEM_PROMPT = (
        ...
        "Use other for content that is not a supported business or financial document, including books, articles, manuals, letters, and general prose. "
        "For other, do not invent transaction data and return line_items as an empty array. "
        "Never fabricate line items, vendors, dates, prices, quantities, or totals that are not supported by the source text. "
