...
# Sensitive‐data scrubber stays tiny
...
def remove_sensitive_data(text):
    ...
    def _repl(m):
        t = m.lastgroup.upper()
        sensitive_info.setdefault(t, []).append(m.group())
        return f'[{t}]'
    sanitized = SENSITIVE_PATTERN.sub(_repl, text)
    return sanitized, sensitive_info
...
def get_original_filename(sanitized_filename: str) -> str:
    ...
    name = UUID_RE.sub('', sanitized_filename)
    u = name.rfind('_')
    return name[:u] + '.' + name[u+1:] if u != -1 else name


# ——————————————————————————————————————————
# CLEAN TEXT (lazy spaCy + ftfy)
# ——————————————————————————————————————————
_PAGE_FOOTER = re.compile(r'(?i)\bpage\s+\d+\s+of\s+\d+\b')
...
def _get_nlp():
    ...
    if _nlp is None:
        import spacy
        # use blank model + only needed components
        _nlp = spacy.blank("en")
        _nlp.add_pipe("sentencizer")   # only sentence boundary detector
    return _nlp

def clean_text(raw_text: str) -> str:
    # import ftfy only when we actually clean
    import ftfy

    # fix encoding + collapse whitespace
    ...
    sentences = [
        ...
        if not _PAGE_FOOTER.search(sent.text)
        and len(sent.text.split()) >= _MIN_WORDS
    ]
