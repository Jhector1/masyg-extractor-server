...
class DocumentService:
    ...
    async def send_document_in_bulk(self, documents: List[Document], share_progress: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create customers and items in bulk (bucketed to respect API limits).
            ...
            for document in documents:
                try:
                    ...
                    # ✅ Build line items with guaranteed ItemRef.value
                    line_items = []
                    ...
                    if not line_items:
                        await self._log(
                            f"❌ No valid line items after resolution for document {document.transaction_id}.", "error"
                        )
                        continue
    ...
    # In: masyg_extractor/integration_qb_v5/intergrate/quickbooks/services/document_service.py
    # Add this helper inside DocumentService (anywhere in the class).

    async def _ensure_item_id(self, i: Item) -> str | None:
        """
        Ensure the Item has an Id:
          1) try by Sku
          2) try by Name
          3) create single item as last resort
        Return the Id or None if not resolvable.
        """
        if getattr(i, "id", None):
            return str(i.id)

        # 1) lookup by Sku (best)
