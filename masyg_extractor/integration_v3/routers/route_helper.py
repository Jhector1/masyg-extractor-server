...
from masyg_extractor.integration_v2.domain.models import *
...
def create_customer(details: Dict[str, Any]) -> Customer:
    """
...
def create_item(line_item: Dict[str, Any]) -> Item:
    """
...
def create_document(txn_id_full: str, group_id: str, details: Dict[str, Any]) -> Document:
    ...
    items = [create_item(item) for item in details.get("line_items", [])]
    return Document(
        customer=customer,
        items=items,
