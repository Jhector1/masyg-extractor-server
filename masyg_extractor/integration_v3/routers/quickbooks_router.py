...
from masyg_extractor.integration_v2.domain.models import Customer, Item, Account, Invoice, SalesReceipt
from masyg_extractor.integration_v2.entity_helper import EntityHelper
from masyg_extractor.integration_v2.intergrate.quickbooks.adapter import QuickBooksClientAdapter
from masyg_extractor.integration_v2.intergrate.quickbooks.services.invoice_service import InvoiceService
from masyg_extractor.integration_v2.intergrate.quickbooks.services.receipt_service import SalesReceiptService
from masyg_extractor.integration_v2.repository.firestore_repository import QuickBooksFirestoreService
from masyg_extractor.integration_v2.routers.route_helper import normalize_payload, handle_quickbooks_request
...
from masyg_extractor.services import log_manager
...
from masyg_extractor.services.progress_log import IntegrationsProgressLog, get_integrations_progress_logger_factory, \
    get_integrations_progress_logger
...
async def send_sales_receipt_route(
    request: Request,
    global_progress: dict = Depends(IntegrationsProgressLog.get_file_progress_dict),
    progress_logger: IntegrationsProgressLog = Depends(get_integrations_progress_logger_factory("invoice-log-message")),
    current_user: dict = Depends(get_current_user_from_cookie),
    ...
    This endpoint:
      - Parses and validates the sales receipt payload.
      - Creates a QuickBooks integration context and client adapter.
      - Uses the SalesReceiptService to send receipts concurrently.
      - Emits progress updates to the client.
    """
    return await handle_quickbooks_request(
        request=request,
        doc_type="SalesReceipt",
        record_key="sales_receipt_data",
        progress_event="quickbooks-sales-receipt-progress",
        current_user=current_user,
        progress_logger=progress_logger,
        global_progress=global_progress,
        service_factory=SalesReceiptService,
        send_method_getter=lambda service: service.send_receipt,
    )
